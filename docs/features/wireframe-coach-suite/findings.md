...
| Decision | Rationale |
|---|---|
| 모더레이터 코치 v1.2: 선정 트리 HTML에 `atf-data` JSON 내장 (ATF v3.0 스키마) | jcurve_ski 어댑터 직결 — E2E 검증에서 수동 변환했던 것을 계약화 |
...
### G6 모더레이터 코치 Sonnet 5 검증 (2026-08-24)
...
- 결론: **모더레이터 코치도 모델 이식 가능** — 두 스킬 모두 Sonnet 5 검증 완료. G6 해소
