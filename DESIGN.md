...
| Error | `--status-error` | `#A52A2A` | 데이터 오류·주의 |
| Neutral | `--status-neutral` | `#5F6670` | 현재 우선 검토 아님·다른 방식 검토 |
...
### 4단계 탐색 요약
- **Structure**: 전체 프로세스(LV4), 전체 후보 업무(LV5), 원본 AI 전환 추천, 모더레이터 기준 충족의 네 수치를 한 흐름으로 보여준다.
- **Behavior**: 수치는 입력 데이터에서 계산한다. 원본 추천과 모더레이터 판단을 합치거나 새 순위로 만들지 않는다.
- **States**: 전체, 원본 추천, 기준 충족, 추가 확인. 선택한 필터는 텍스트·테두리·`aria-pressed`로 함께 표시한다.
- **Layout**: 375px에서는 네 단계를 세로로 쌓고 연결선 대신 단계 번호로 순서를 유지한다.

### 업무 카드
- **Structure**: ID 보조 표기, 제목, 근거, 상태 배지, 범위 요약
- **Variants**: 기준 충족, 추가 확인, 다른 방식 함께 검토, 현재 우선 검토 아님
- **Depth**: 바깥 카드 한 겹만 사용하고 카드 안에 또 다른 장식 카드 체계를 만들지 않는다.
- **Language**: `탈락`, `제외`, `부적합`처럼 팀 선택을 닫는 표현을 쓰지 않는다. 상태는 현재 탐색 우선순위일 뿐 원본 업무 보존 여부가 아니다.
...
- **Gate**: 판정 완료에는 팀 선택 확인과 선택 이유가 필요하다. 모더레이터 기준 미충족 업무를 고르면 팀의 예외 선택 이유를 별도로 남긴다.

### LV5 전체 와이어프레임
- **Structure**: 사람 시작 경계 → 실행 노드 → 사람 최종 책임을 한 본선에 표시하고, 각 노드에 소속 LV6와 제안 환경을 함께 쓴다.
- **Skill boundary**: 추천 스킬의 노드는 `skillHintId`로 묶되, 자동화 밖 사람 노드를 지도에서 삭제하지 않는다.
- **Layout**: 1280px에서는 다섯 열 두 줄, 768px 이하에서는 한 열 세로 흐름으로 재배치한다. 긴 전체 지도를 가로 스크롤로 축소하지 않는다.

### 스킬 추천·템플릿 힌트 카드
- **Structure**: 추천 순서, 스킬명, 한 줄 정의, 입력, 출력, 사람 확인을 먼저 보여주고 상세 페이지에서 실행 순서·규칙·예외·테스트·미확인 질문을 펼친다.
- **Hierarchy**: 첫 추천 한 장만 강조하고 나머지는 같은 깊이의 종이 카드로 유지한다.
- **Language**: 완성된 스킬처럼 단정하지 않고 `힌트`, `제작 전에 확인`으로 인수인계 상태를 명시한다.
